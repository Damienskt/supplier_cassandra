...
public class NewOrderTransaction {
    ...
    private String SELECT_STOCK;
    private String SELECT_ITEM;
    private String SELECT_CUSTOMER;

    private String INSERT_ORDER;
    private String INSERT_ORDERLINE;

    private String UPDATE_CUSTOMER_LAST_O_ID;
    private String UPDATE_DISTRICT_NEXT_O_ID;
    private String UPDATE_STOCK;

    public NewOrderTransaction(Session session, String keySpace) {
        this.KEY_SPACE_WITH_DOT = keySpace + ".";

        this.SELECT_DISTRICT = " SELECT D_TAX, D_NEXT_O_ID FROM "
                + KEY_SPACE_WITH_DOT + Table.TABLE_DISTRICT + " WHERE D_W_ID = ? AND D_ID = ?; ";
        this.SELECT_WAREHOUSE = " SELECT W_TAX FROM "
                + KEY_SPACE_WITH_DOT + Table.TABLE_WAREHOUSE + " WHERE W_ID = ?; ";
        this.SELECT_STOCK = " SELECT * FROM "
                + KEY_SPACE_WITH_DOT + Table.TABLE_STOCK + " WHERE S_W_ID = ? AND S_I_ID = ?; ";
        this.SELECT_ITEM = " SELECT I_NAME, I_PRICE FROM "
                + KEY_SPACE_WITH_DOT + Table.TABLE_ITEM + " WHERE I_ID = ?; ";
        this.SELECT_CUSTOMER = " SELECT C_LAST, C_CREDIT, C_DISCOUNT FROM "
                + KEY_SPACE_WITH_DOT + Table.TABLE_CUSTOMER + " WHERE C_W_ID = ? AND C_D_ID = ? AND C_ID = ?; ";

        this.INSERT_ORDER = " INSERT INTO " + KEY_SPACE_WITH_DOT + Table.TABLE_ORDER + " ("
                + " O_W_ID, O_D_ID, O_ID, O_C_ID, O_CARRIER_ID, "
                + " O_OL_CNT, O_ALL_LOCAL, O_ENTRY_D ) "
                + " VALUES (?, ?, ?, ?, ?, ?, ?, ?); ";
        this.INSERT_ORDERLINE = " INSERT INTO " + KEY_SPACE_WITH_DOT + Table.TABLE_ORDERLINE + " ("
                + " OL_W_ID, OL_D_ID, OL_O_ID, OL_NUMBER, OL_I_ID, "
                + " OL_DELIVERY_D, OL_AMOUNT, OL_SUPPLY_W_ID, OL_QUANTITY, OL_DIST_INFO ) "
                + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?); ";

        this.UPDATE_CUSTOMER_LAST_O_ID = " UPDATE " + KEY_SPACE_WITH_DOT + Table.TABLE_CUSTOMER
                + " SET C_LAST_O_ID = ?, C_O_ENTRY_D = ?, C_O_CARRIER_ID = ? "
                + " WHERE C_W_ID = ? AND C_D_ID = ? AND C_ID = ?;";
        this.UPDATE_DISTRICT_NEXT_O_ID = " UPDATE " + KEY_SPACE_WITH_DOT + Table.TABLE_DISTRICT
                + " SET D_NEXT_O_ID = ? WHERE D_W_ID = ? AND D_ID = ?; ";
        this.UPDATE_STOCK = " UPDATE " + KEY_SPACE_WITH_DOT + Table.TABLE_STOCK
